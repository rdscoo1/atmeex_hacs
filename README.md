...
4. Выберите способ авторизации:
   - **Email и пароль** — введите email и пароль от аккаунта Atmeex Cloud
   - **Телефон и SMS-код** — введите номер телефона, затем SMS-код, который придёт на него
5. Интеграция автоматически обнаружит все ваши устройства Atmeex.
...
- **Включить сенсор CO2** (вкл/выкл): Показывать датчик уровня CO2 для каждого устройства. Отключите, если ваше устройство не оснащено датчиком CO2.
...
| `sensor` | `sensor.bedroom_breezer_co2` | Уровень CO2 в ppm (опционально) |
| `sensor` | `sensor.bedroom_breezer_humidity` | Влажность в комнате |
| `sensor` | `sensor.bedroom_breezer_inlet_temperature` | Температура приточного воздуха |
...
3. Нажмите **Настроить** (или следуйте уведомлению о повторной авторизации)
4. Для аккаунтов с email: введите email и пароль заново
5. Для аккаунтов с телефоном: подтвердите отправку SMS-кода — новое сообщение придёт только после вашего подтверждения

### Пропадает сенсор CO2

Отключите сенсор CO2 в опциях интеграции, если ваше устройство не оснащено датчиком CO2.
...
```bash
# Запустить все тесты (224+, всегда должны быть зелёными)
